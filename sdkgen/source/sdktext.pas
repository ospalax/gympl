...
if jmeno=filerr then
writeln(f,' Soubor^neexistuje! ');

if jmeno=daterr then
writeln(f,' Soubor je necitelny! ');
...
if jmeno=annsave then
writeln(f,' Zadani je ulozeno! ');

if jmeno=annunsave then begin
writeln(f,' Zadani nemuze byt ');
writeln(f,' ulozeno! ');
end;
...
writeln(f,'bez souhlasu autora publikovany.');
writeln(f);
...
if jmeno=attmuch then begin
writeln(f,' Zadani ma vicero ');
writeln(f,' reseni! ');
end;
...
var f:SearchRec;
    ...
    if doserror<>0 then mkdir('data\sdk');
	 findfirst('data\txt',16,f);
    if doserror<>0 then mkdir('data\txt');
    end;
